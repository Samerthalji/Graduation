...
export default function SignUpPage() {
  ...
  const field = (label, name, type = 'text', placeholder = '') => (
    <div>
      <label className="block text-sm text-start font-semibold text-gray-700 mb-1 px-3">{label}</label>
      ...
      {errors[name] && <p className="text-red-500 text-xs mt-1 px-3">{errors[name]}</p>}
    </div>
  );
  ...
  return (
    <div className="bg-gray-100 min-h-screen flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl overflow-hidden max-w-5xl w-full flex flex-col md:flex-row min-h-[600px]">
        ...
        <div className="w-full md:w-7/12 p-3 md:p-10 bg-white text-right" dir="ltr">
          <div className="flex flex-col items-center mb-6 gap-1">
            <img src="/5.png" alt="Doroop Logo" className="w-1/3" />
            <h2 className="text-3xl font-extrabold text-blue-800 mt-4 mb-3">Sign Up</h2>
          ...
          <form className="space-y-3" onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {field('Phone number', 'phone', 'tel', '077*******')}
              {field('Location', 'location', 'text', 'Jordan-Amman')}
            </div>
            <button type="submit"
              className="w-full bg-white hover:bg-blue-800 text-blue-800 hover:text-white border-2 border-blue-800 font-bold py-3.5 rounded-xl shadow-lg shadow-indigo-200 transform transition active:scale-95 mt-10">
              Sign Up
            </button>
          </form>
        ...
        <div className="hidden md:flex relative w-full md:w-5/12 bg-indigo-900 text-white p-8 flex-col justify-end overflow-hidden">
          <img src="/pexels-benjamin-adjei-abayie-2158492422-36659831.jpg" alt="Background"
            className="absolute inset-0 w-full h-full object-cover opacity-60" />
          <div className="absolute inset-0 bg-gradient-to-t from-indigo-950 via-transparent to-transparent"></div>
          <div className="relative z-10 text-left">
            <h1 className="text-4xl font-bold leading-tight">Your skills deserve <br />
